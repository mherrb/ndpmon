...
int read_manuf_file(char *filename, manufacturer_t **list)
{
	...
	while( fgets(buffer, BUFFER_SIZE, f) != NULL)
	{
		...
		else if( (buffer[8] == ':') || (buffer[2] == '-') )
		{
			continue;
			/*
			if (sscanf(buffer, "%2x:%2x:%2x:%2x:%2x:%2x/%2x %" MANUFACTURER_NAME_LEN_FSTR "s",&first, &second, &third, &fourth, &fifth, &sixth, &len, manuf_name) != EOF)
			{
				char code[MANUFACTURER_CODE_SIZE]; 
				fprintf(stderr,"\n%s", buffer);
				sprintf(code,"%.2x:%.2x:%.2x:%.2x:%.2x:%.2x", first,second,third, fourth, fifth, sixth);
				fprintf(stderr, "Add code %s, name %s\n", code,manuf_name);
			}
			*/
		}	
